...
export function CausalGraph({ snapshot, nodes }: CausalGraphProps) {
  ...
  return (
    <div className="graph-stage" aria-label="Kanıtlı nedensel ilişki grafiği">
      <svg viewBox="0 0 360 360" role="img">
        <title>Kanıtlı nedensel ilişki grafiği</title>
        <defs>
        ...
        {snapshot.nodes.slice(0, 6).map((node, index) => {
          ...
          return (
            <g key={node.id} transform={`translate(${position.x} ${position.y})`}>
              <circle r="28" className="graph-node" />
              <text textAnchor="middle" y="3" className="graph-node__value">KANIT</text>
        ...
        <g transform="translate(180 180)">
          ...
          <text textAnchor="middle" y="13" className="graph-core__tick">TRUTH GATE</text>
        </g>
      </svg>
    </div>
  );
